...
router.post('/:productID/rating', authMiddleware, [
    ...
    try {
        ...
        if (product) {
            const alreadyReview = product.reviews.find((r) => r.user.toString() === req.findUser.id.toString())
            ...
            product.rating = Number(product.reviews.reduce((rate, product) => product.rating + rate, 0) / product.numReviews)
...
router.get('/productDetail/:id', async (req, res) => {
    try {
        ...
        if (data.length === 0)
            return res.status(400).json({ error: "sorry! we haven't found such a product 😢" })
        res.json({ data })
    } catch (e) {
        console.log(e)
        res.json({ error: e }).status(500)
...
router.post('/addCart/:productId', authMiddleware, async (req, res) => {
    ...
    try {
        ...
        const checkProduct = already.products.find((p) => p.productId.toString() === productId.toString())
        if (checkProduct) {
...
router.delete('/deleteCart/:productId', authMiddleware, async (req, res) => {
    ...
    try {
        ...
        pro.products = pro.products.filter((item) => item.productId != req.params.productId)
