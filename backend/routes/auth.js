...
router.post("/signUp", [
    ...
    try {
        ...
        if (findUser)                                                  // if user already exist then throws a NOT ACCEPTABLE status (406)
...
router.post('/login',
    [
        ...
        try {
            ...
            if (!findUser)
                return res.status(404).json({ error: email + " not found such user 😢" })      // if user is not found, them it will throw a NOT FOUND status (404)
