...
router.post(
  ...
  async (req, res) => {
    const { shopName, contactNumber, email, password, address, pincode } =
      req.body;
    const error = validationResult(req);

    if (!error.isEmpty()) return res.status(400).json({ error: error.array() }); // if error is occurred then throw a BAD REQUEST status (400)

    const checkUSerEmail = await registration.findOne({ email: email });
    const checkUSerPhoneNumber = await registration.findOne({
    ...
    try {
      ...
      const findSeller = await sellerOrders.findOne({
      ...
      res.status(200).json({ sellerToken: jwt });
    } catch (e) {
...
router.post("/sellerLogin", [body("email").isEmail()], async (req, res) => {
  const error = validationResult(req);
  ...
  try {
    ...
    if (!seller)
      return res
        .status(404)
        .json({ error: "check your email id and try again" });
